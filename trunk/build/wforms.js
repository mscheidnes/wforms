/*** 
	...
	Build $Wed, 20 Mar 2013 20:58:11 UTC$
...
/*
    if(wFORMS.behaviors.repeat) {
        var __onIdChange = wFORMS.behaviors.repeat.onIdChange;
        wFORMS.behaviors.repeat.onIdChange = function(n, oldId, newId) {

            var elems  = n.querySelectorAll("option[data-filter-dependent~="+oldId+"], 
                                             select[data-filter-dependent~="+oldId+"], 
                                             input[data-filter-dependent~="+oldId+"],
                                             option[data-filter-control~="+oldId+"], 
                                             select[data-filter-control~="+oldId+"], 
                                             input[data-filter-control~="+oldId+"],
                                             option[data-filter-include~="+oldId+"],
                                             input[data-filter-exclude~="+oldId+"],
                                             option[data-filter-include~="+oldId+"],
                                             input[data-filter-include~="+oldId+"]"
                                            );
            if(elems.length) {
                elems.forEach(function(elem) {

                    // check if elem belongs to same repeated section
                    if(wFORMS.behaviors.repeat.inSameScope(elem, n)) {
                        wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-dependent', '#'+oldId, '#'+newId);
                        wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-control', '#'+oldId, '#'+newId);
                        wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-include', '#'+oldId, '#'+newId);
                        wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-exclude', '#'+oldId, '#'+newId);
                    } else {
                        // check if elem belongs to source repeated section, if yes skip.
                        if(wFORMS.behaviors.repeat.inSourceScope(elem, n)) {
                            // nothing to do.   
                        } else {
                            // otherwise, append new ID to data-filter-* selector.
                            wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-dependent', '#'+oldId, '#'+oldId+','+'#'+newId);
                            wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-control', '#'+oldId, '#'+oldId+','+'#'+newId);
                            wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-include', '#'+oldId, '#'+oldId+','+'#'+newId);
                            wFORMS.behaviors.dependent_list.replaceSelectorIn('data-filter-exclude', '#'+oldId, '#'+oldId+','+'#'+newId);
                        }
                    }
                });
            }
            __onIdChange(n, oldId, newId);
        }
    }
*/
    b.onApply();
